...
(function () {
    ...
    const PREVIEW_STYLE = {
        ...
        styleRules: [
            {
                predicate: (properties) => properties.styleName === 'preview-border',
                style: {
                    stroke: true,
                    strokeColor: '#ffffff',
                    strokeLinecap: 'round',
            ...
            {
                predicate: (properties) => properties.styleName === 'preview',
                style: {
                    stroke: true,
                    strokeColor: '#ff2424',
    ...
    class E85 extends WMEBase {
        ...
        onSegment(event, element, model) {
            ...
            if (this.canEditSegment(model)) {
                ...
                if (this.settings.get('microDoglegs', 'enabled')) {
                    let doglegButton = panel.addButton('M', this.buttons.M.title, this.buttons.M.description, () => this.removeMicroDoglegs(model));
        ...
        onSegments(event, element, models) {
            ...
            if (this.settings.get('microDoglegs', 'enabled')) {
                panel.addButton('M', this.buttons.M.title, this.buttons.M.description, () => this.removeMicroDoglegsMultiple(models));
            }
                ...
                if (connections.indexOf(second.id) !== -1) {
                    ...
                    for (let key in settingsButtons) {
                        if (settingsButtons.hasOwnProperty(key)) {
                            ...
                            if (angle === 0)
                                continue;
                            let btn = panel.addButton(key, `\u2221${angle}\u00B0`, `\u2221${angle}\u00B0`, () => this.alignStreetGeometry(first, second, angle));
                            let el = btn.html();
         ...
         */
        computeAlignment(first, second, angle) {
            let A, B, C;
